...
print_header_line() {
    printf_line "%-5s | %-6s | %-${MAX_MODEL_NAME_LENGTH}s | %-8s | %-15s | %-16s | %-7s | %-7s | %-7s | %-10s | %-12s | %-10s | %-10s | %s\n" \
        " ct M" " ct T" "Model" "num_ctx" "  RAM  /  SIZE" "Processor" "T Dur" "L Dur" "P Dur" "Prompt N" "Prompt Rate" "Eval N" "Eval Dur" "Eval Rate"
}
...
for MODEL in "${MODELS[@]}"; do
	...
	# Calculate counters
        ((GLOBAL_RUN++))
	ITER_LABEL=$(printf "%2d/%-2d | %2d/%-2d" "$iter" "$MAX_ITER" "$GLOBAL_RUN" "$TOTAL_RUNS")

	# print proccessing line
	printf "%-14s | %-${MAX_MODEL_NAME_LENGTH}s | %-8s | %15s | %-16s | %-7s | %-7s | %-7s | %-10s | %-12s | %-10s | %-10s | %s" \
	...
	# show benchmark results
        printf_line "\r%-14s | %-${MAX_MODEL_NAME_LENGTH}s | %-8s | %15s | %-16s | %-7s | %-7s | %-7s | %-10s | %-12s | %-10s | %-10s | %s\n" \
            "$ITER_LABEL" "$MODEL_SHORT" "$ctx" \
...
#TODO gesezte context token werden nicht immer akzeoptiert
# gesetzt 8192 aber model lt ollama ps nur mit 4096 geladen
