...
SHOW_CPU_GPU_LOAD_DELAY_DELAY=10


#GPU_USAGE_THRESHOLD=100					# Default for minimum GPU usage
#CPU_USAGE_THRESHOLD=100					# Default for minimun CPU usage



			# Default for waiting llm model fully loaded
...
if [ -n "$SHOW_CPU_GPU_LOAD_DELAY" ]; then
    PROMPT_BENCHMARK="1+1"
    MAX_TIME=$SHOW_CPU_GPU_LOAD_DELAY
fi
...
for MODEL in "${MODELS[@]}"; do
	...
	if [[ $CONTEXT_LIMIT == true ]] || [[ $RAM_LIMIT == true ]] || [[ $BREAK_MODEL_LOOP == true ]]; then
	    ((iter++))
	    ollama stop "$MODEL"        
	    continue
	fi
	...
	#TODO
        if [ -z "$OUTPUT" ]; then
            ...
            "$ITER_M" "$ITER_T" "$MODEL_SHORT" "No response (timeout after ${MAX_TIME}s or connection error) ($MODEL )" | tee -a "$LOG_FILE"
            ((iter++))
            ollama stop "$MODEL"
            continue
        fi
