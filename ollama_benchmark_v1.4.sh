...
PROMPT_BENCHMARK="Schreibe einen 100-Wörter-Artikel über die Zukunft der KI. Nutze technische Begriffe und strukturierte Absätze."                 # Benchmark-Prompt
...
for MODEL in "${MODELS[@]}"; do
    ...
    while [ $iter -le $MAX_ITER ]; do
        ...
        if [[ "$VERBOSE_MODE" -eq 1 ]]; then
            ...
            VERBOSE_FILE="${VERBOSE_DIR}/${SAFE_MODEL}_${ctx}.json"
            echo "$OUTPUT" | python3 -c "import sys,json; print(json.dumps(json.load(sys.stdin), indent=2))" \
                | jq --arg zahl "${ctx}" 'del(.context) | . + {ctx_token: ($zahl | tonumber)}' > "$VERBOSE_FILE" 2>/dev/null                 
        fi
