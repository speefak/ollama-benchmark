...
if [ ${#MODELS[@]} -eq 0 ]; then
    ...
    while [ ${#MODELS[@]} -eq 0 ]; do
        echo -n "Select model(s) (z.B. 1 3 oder 1-5): "
        read -r SELECTION
...
print_header_line() {
    printf_line "%-2s | %-${MAX_MODEL_NAME_LENGTH}s | %-8s | %-15s | %-16s | %-7s | %-7s | %-7s | %-10s | %-12s | %-10s | %-10s | %s\n" \
        " #" "Model" "num_ctx" "  RAM  /  SIZE" "Processor" "T Dur" "L Dur" "P Dur" "Prompt N" "Prompt Rate" "Eval N" "Eval Dur" "Eval Rate"
#    print_line "$SEP"
}
...
for MODEL in "${MODELS[@]}"; do
	...
	echo -n "  Iteration $iter | $MODEL | ctx=$ctx | running..."
	...
	# show benchmark results
        printf_line "\r%2s | %-${MAX_MODEL_NAME_LENGTH}s | %-8s | %15s | %-16s | %-7s | %-7s | %-7s | %-10s | %-12s | %-10s | %-10s | %s\n" \
