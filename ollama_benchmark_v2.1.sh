...
print_header_line() {
    printf_line "%13s | %-${MAX_MODEL_NAME_LENGTH}s | %-8s | %-15s | %-16s | %-7s | %-7s | %-7s | %-10s | %-12s | %-10s | %-10s | %s\n" \
        "ctx | run " "Model" "num_ctx" "  RAM  /  SIZE" "Processor" "T Dur" "L Dur" "P Dur" "Prompt N" "Prompt Rate" "Eval N" "Eval Dur" "Eval Rate"
}
...
for MODEL in "${MODELS[@]}"; do
	...
	ITER_LABEL="${iter}/${MAX_ITER} | ${GLOBAL_RUN}/${TOTAL_RUNS}"
        printf_line "\r%13s | %-${MAX_MODEL_NAME_LENGTH}s | %-8s | %15s | %-16s | %-7s | %-7s | %-7s | %-10s | %-12s | %-10s | %-10s | %s\n" \
