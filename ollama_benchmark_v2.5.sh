...
for MODEL in "${MODELS[@]}"; do
	...
	# ollama benchmark
	    ...
	    printf "\r\033[K%7s | %7s | %-${MAX_MODEL_NAME_LENGTH}s | %6s / %s\n" \
            "$ITER_M" "$ITER_T" "$MODEL_SHORT" "$PS_CONTEXT_CTX" "$ERROR ( $MODEL )" | tee -a "$LOG_FILE"
            RAM_LIMIT=true
            ((iter++))
            continue
        fi
...
echo "$ctx $PS_CONTEXT_CTX" >> fuck

        # Check for LLM context limit, 
        if [ "$ctx" -gt "$PS_CONTEXT_CTX" ]; then
            printf "\r\033[K%7s | %7s | %-${MAX_MODEL_NAME_LENGTH}s | %6s / %-24s | %4s / %-4s | %-7s | %-7s | %-7s | %-10s | %-12s | %-10s | %-10s | %s\n" \
            "$ITER_M" "$ITER_T" "$MODEL_SHORT" "$PS_CONTEXT_CTX" "Context Limit reached"
            CONTEXT_LIMIT=true
            continue
        fi 
