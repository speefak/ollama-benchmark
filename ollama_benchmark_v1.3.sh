...
#   ./ollama-ctx-benchmark.sh -m qwen3:4b llama3:8b -cr 50% -l 5 -t 300
#
# Requirements:
...
PROMPT_BENCHMARK="Schreibe einen 1000-Wörter-Artikel über die Zukunft der KI. Nutze technische Begriffe und strukturierte Absätze."                 # Benchmark-Prompt
...
usage() {
    ...
    echo "  $0 -m qwen3:4b -cs 4096 -cr 1024 -l 5"
    exit 0
}
...
echo "Erhöhung    : $CTX_RAISE_INFO"
echo "Durchgänge  : $MAX_ITER"
echo "Timeout     : ${MAX_TIME}s"
echo "Prompt      : \"$PROMPT_BENCHMARK\""
echo ""

SEP="$(printf '%0.s-' {1..210})"

print_header() {
    printf "%-10s | %-${MAX_MODEL_NAME_LENGTH}s | %-8s | %-10s | %-16s | %-10s | %-10s | %-10s | %-10s | %-12s | %-10s | %-10s | %s\n" \
...
for MODEL in "${MODELS[@]}"; do
    ...
    while [ $iter -le $MAX_ITER ]; do
        ...
        printf "\r%-10s | %-${MAX_MODEL_NAME_LENGTH}s | %-8s | %-10s | %-16s | %-10s | %-10s | %-10s | %-10s | %-12s | %-10s | %-10s | %s\n" \
