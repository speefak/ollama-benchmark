...
#   -rc,  --reload-cuda	        Reload CUDA driver"
...
SEP=$(printf '─%.0s' $(seq 1 184))			# Seperatorline lenght
...
usage() {
    ...
    echo "  -f,   --fastmode SEC             CPU/GPU load benchmark (Default: \"$FASTMODE_TIMEOUT_DEFAULT\")"
    ...
    echo "  -smi, --show-model-info VALUE    Show model information ( 1,2,3: columnsort)"
    echo ""
    echo "Examples:"
...
while [[ $# -gt 0 ]]; do
    ...
    case $FLAG in
       ...
       -gl|--gpu-load)
            if [[ "$1" =~ ^[0-9]+$ ]] && [[ "$1" -ge 0 && "$1" -le 100 ]]; then
                GPU_USAGE_THRESHOLD="$1"
            else
                echo "❌ Invalid value for -gl: $1 (must be 1-100)"; usage
            fi
            shift
            ;;
       -cl|--cpu-load)
            if [[ "$1" =~ ^[0-9]+$ ]] && [[ "$1" -ge 0 && "$1" -le 100 ]]; then
                ...
                echo "❌ Invalid value for -cl: $1 (must be 1-100)"; usage
            fi
            shift
            ;;           
        -p|--prompt)
        ...
        -t|--timeout)
            if [[ "$1" =~ ^[0-9]+$ ]]; then
                ...
                echo "❌ Invalid value for -tr: $1"; usage
            fi
            shift
            ;; 
        -v|--verbose)
	...
	-f|--fast)
		...
		FASTMODE=true
		  MAX_TIME=$FASTMODE_TIMEOUT_DEFAULT
            else
                echo "❌ Invalid value for -f: $1"; usage
            fi
            ;;      
        *)
...
# Get hardwareinfo
GPU_MODEL=$(nvidia-smi --query-gpu=name --format=csv,noheader 2>/dev/null || echo "N/A")
...
print_header_line() {
    printf_line "%7s | %7s | %-${MAX_MODEL_NAME_LENGTH}s | %6s | %-15s | %4s / %-4s | %-7s | %-7s | %-7s | %-10s | %-12s | %-10s | %-10s | %s\n" \
        "iter M" "iter T" "Model" "ctx " "  RAM  /  SIZE" "CPU" "GPU" "T Dur" "L Dur" "P Dur" "Prompt N" "Prompt Rate" "Eval N" "Eval Dur" "Eval Rate"
}
...
for MODEL in "${MODELS[@]}"; do
	...
	read PS_CPU PS_GPU < <(awk '{
		...
		# single format: "40%" CPU or GPU
		if ($i ~ /^[0-9]+%$/) {
		    gsub(/%/,"",$i)
		    if ($(i+1) == "CPU")  { print $i, 0; exit }
		    if ($(i+1) == "GPU")  { print 0, $i; exit }
		}
	    }
	}' <<< "$PS_LINE")

	# Skip loop processing for zero output
        if [[ -z "$OUTPUT" ]]; then    
            printf "\r\033[K%7s | %7s | %-${MAX_MODEL_NAME_LENGTH}s | %6s | %s\n" \
            "$ITER_M" "$ITER_T" "$MODEL_SHORT" "$ctx" "No response (timeout after ${MAX_TIME}s or connection error) ($MODEL )" | tee -a "$LOG_FILE"
            ((iter++))
	...
	# Skip loop proccessing for ollama errors
	...
	# Skip value processing in fastmode to avoid parsing errors
        if [ -z "$FASTMODE" ]; then
        ...
        if [[ -z "$PS_CONTEXT_CTX" ]] || [ "$ctx" -gt "$PS_CONTEXT_CTX" ]; then
            printf "\r\033[K%7s | %7s | %-${MAX_MODEL_NAME_LENGTH}s | %6s | %-50s\n" \
            "$ITER_M" "$ITER_T" "$MODEL_SHORT" "$ctx" "Context Limit reached ($PS_CONTEXT_CTX)" 
            BREAK_MODEL_LOOP=true
            ollama stop "$MODEL"  
            continue
        fi

	# Check for GPU/CPU LLM load
	if [[ -n $GPU_USAGE_THRESHOLD ]] ;then
		# Mininal GPU load threshold
		if   [[ "$PS_GPU" -lt "$GPU_USAGE_THRESHOLD" ]]; then
		    BREAK_MODEL_LOOP=true
		    # printf info line
		printf "\r\033[K%7s | %7s | %-${MAX_MODEL_NAME_LENGTH}s | %6s | %-26s | %-7s | %-7s | %-10s | %-12s | %-10s | %-10s | %s\n" \
		    "$ITER_M" "$ITER_T" "$MODEL_SHORT" "$ctx" "Minimal GPU load limit reached (minimum gpu load $PS_GPU of ${GPU_USAGE_THRESHOLD}%)" \
		    "" "" "" "" "" "" ""
		    ((iter++))
		    ollama stop "$MODEL"
		    continue
	...
	if [[ -n $CPU_USAGE_THRESHOLD ]] ; then
		if [[ "$PS_CPU" -lt "$CPU_USAGE_THRESHOLD" ]]; then
		    BREAK_MODEL_LOOP=true
		    # printf info line
		    printf "\r\033[K%7s | %7s | %-${MAX_MODEL_NAME_LENGTH}s | %6s | %-29s | %-7s | %-7s | %-7s | %-10s | %-12s | %-10s | %-10s | %s\n" \
		    "$ITER_M" "$ITER_T" "$MODEL_SHORT" "$ctx" "Minimal CPU load limit reached (minimum cpu load $PS_CPU of ${CPU_USAGE_THRESHOLD}%)" \
		    "" "" "" "" "" "" "" ""
		    ((iter++))
		    ollama stop "$MODEL"
		    continue
	...
	# show benchmark results
        printf_line "\r%7s | %7s | %-${MAX_MODEL_NAME_LENGTH}s | %6s | %15s | %4s / %-4s | %-7s | %-7s | %-7s | %-10s | %-12s | %-10s | %-10s | %s\n" \
            "$ITER_M" "$ITER_T" "$MODEL_SHORT" "$PS_CONTEXT_CTX" \
