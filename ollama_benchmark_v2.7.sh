...
while [[ $# -gt 0 ]]; do
    ...
    case $FLAG in
      ...
      -smi|--show-model-files)
        ...
        -f|--fast)
            if [[ -n "$2" && "$2" =~ ^[0-9]+$ ]]; then
                SHOW_CPU_GPU_LOAD_DELAY="$2"  # Wert verwenden
                shift 2  # Option + Wert entfernen
            else
                SHOW_CPU_GPU_LOAD_DELAY=5  # Default
                shift  # Nur Option entfernen
            fi
            ;;
        *)
...
#TODO	# wait if $MODEL with ctx value is loaded  #check via ollama ps
...
#TODO option -f soll mit und ohne gehen - aktuel fehler ohne zahl 
...
#TODO skip when no gpu / cpu use (CPU GPU values emptpy )
