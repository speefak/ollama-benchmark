...
usage() {
    ...
    echo "  -f,   --fast TEXT                Fast CPU/GPU load benchmark"
...
print_header_line() {
    printf_line "%-6s | %-6s | %-${MAX_MODEL_NAME_LENGTH}s | %-13s | %-15s | %-16s | %-7s | %-7s | %-7s | %-10s | %-12s | %-10s | %-10s | %s\n" \
        "iter M" "iter T" "Model" "ctx S / ctx L" "  RAM  /  SIZE" "Processor" "T Dur" "L Dur" "P Dur" "Prompt N" "Prompt Rate" "Eval N" "Eval Dur" "Eval Rate"
}
...
for MODEL in "${MODELS[@]}"; do
    ...
    MODEL_SIZE=$(ollama list | grep -x "$MODEL" | awk '{printf "%3s %s\n", $3, $4}')
    MODEL_ID=$(ollama list | grep -x "$MODEL" | awk '{printf $2}')
	...
	# Calculate counters
        ((GLOBAL_RUN++))
	ITER_LABEL=$(printf "%3d/%-2d | %2d/%-2d" "$iter" "$MAX_ITER" "$GLOBAL_RUN" "$TOTAL_RUNS")

	# print proccessing line
	printf "%-15s | %-${MAX_MODEL_NAME_LENGTH}s | %-13s | %15s | %-16s | %-7s | %-7s | %-7s | %-10s | %-12s | %-10s | %-10s | %s" \
        "$ITER_LABEL" "$MODEL_SHORT" "$ctx"	

	# Set Vars for fast benchmark	
	if [ -n "$SHOW_CPU_GPU_LOAD" ]; then	
	    PROMPT_BENCHMARK=""
	...
	# ollama benchmark
        ...
        if [ -z "$OUTPUT" ] && [ -z "$SHOW_CPU_GPU_LOAD" ]; then
	...
	# show benchmark results
        printf_line "\r%-15s | %-${MAX_MODEL_NAME_LENGTH}s | %-13s | %15s | %-16s | %-17s | %-7s | %-7s | %-10s | %-12s | %-10s | %-10s | %s\n" \
            "$ITER_LABEL" "$MODEL_SHORT" "$ctx/$PS_CONTEXT_CTX" \
	...
	ollama stop "$MODEL"        
        ...
        sleep $OLLAMA_MODEL_UNLOD_DELAY
...
#TODO mex ctx_num von model vor eingabepromt herausfinden und loop bei erreichen abbrechen
